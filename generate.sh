...
if command -v yq &> /dev/null; then
    ...
    SOCKET=$(yq eval '.docker.socket' "$CONFIG_YML" 2>/dev/null || echo "/var/run/docker.sock")
    ...
    SOCKET=$(docker run --rm -i -v "$SCRIPT_DIR:/workdir" mikefarah/yq eval '.docker.socket' /workdir/config.yml 2>/dev/null || echo "/var/run/docker.sock")
...
output_content="${output_content//\{\{SOCKET\}\}/$SOCKET}"
